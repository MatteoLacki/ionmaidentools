...
Absorbs the procedural body of the old Snakemake `sage` rule: prints the Sage
version, runs the search, asserts the expected output files exist, and writes a
run_info.json describing the invocation (kept for future regression-DB wiring —
see TODO in the necromerge2 necroflow pipeline module).
"""
import argparse
import json
import subprocess
...
def main():
    ...
    p.add_argument("config_path", type=Path, help="Path to the Sage JSON config.")
